...
export default function PaymentsManagement() {
  ...
  const handleOpenPaymentModal = (record) => {
    ...
    setModalMethod(record.payment_method || 'Cash');
  };
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h1 className="heading-1 flex items-center gap-2" style={{ marginBottom: 0 }}><CreditCard /> Payments & Invoices</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-3)', marginBottom: 'var(--space-4)' }}>
        <div style={{ padding: 'var(--space-3)', backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
          <div className="text-light text-sm">Collected (Month {filterMonth}/{filterYear})</div>
          <div style={{ fontSize: '1.4rem', fontWeight: 700, color: 'var(--color-success)' }}>${monthlyCollected.toFixed(2)}</div>
        </div>
        <div style={{ padding: 'var(--space-3)', backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)' }}>
          <div className="text-light text-sm">Owed (Month {filterMonth}/{filterYear})</div>
        ...
        <div style={{ padding: 'var(--space-3)', backgroundColor: 'rgba(239, 68, 68, 0.05)', border: '1px solid rgba(239, 68, 68, 0.3)', borderRadius: 'var(--radius-lg)' }}>
          <div className="text-light text-sm" style={{ color: 'var(--color-danger)' }}>Historical Debt (Previous Months)</div>
      ...
      <Card className="mb-4">
        <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div className="form-group" style={{ marginBottom: 0, minWidth: '200px', flex: 1 }}>
            ...
            >
              <option value="">All students (View filtered month)</option>
              {students.map(s => (
          ...
          {/* Enable date filters if no student is selected */}
          {!selectedStudentId && (
      ...
      {showAdd && (
        <Card className="mb-4">
          <h2 className="heading-2">Create Payment Record</h2>
        ...
        <div style={{ display: 'grid', gap: 'var(--space-2)' }}>
          {payments.length === 0 && (
            <p className="text-light">
              {selectedStudentId ? 'No payments recorded for this student.' : 'No invoices recorded for this month.'}
            </p>
          )}
          {payments.map(p => (
            <div key={p.id} className="flex justify-between items-center flex-wrap gap-2" style={{ 
              ...
              <div>
                ...
                <div className="text-light text-sm">
                  ${p.amount} • Month {p.month}/{p.year}
                </div>
                {p.paid && (
                  <div style={{ marginTop: '4px' }}>
                    <span className="badge badge-success">
                      Paid on {formatPaymentDate(p.date_paid)} via {p.payment_method || 'Cash'}
                    </span>
                  </div>
                )}
              </div>
              ...
              >
                {p.paid ? 'Details / Edit' : 'Mark as Paid'}
              </Button>
            </div>
          ))}
        </div>
        ...
        <div className="modal-backdrop" onClick={() => setPayingRecord(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h2 className="heading-2" style={{ marginBottom: 0 }}>Register / Edit Payment</h2>
            ...
            <form onSubmit={handleSavePaymentDetails}>
              <div className="modal-body">
                <div style={{ marginBottom: 'var(--space-4)', backgroundColor: 'var(--color-bg)', padding: 'var(--space-3)', borderRadius: 'var(--radius-md)' }}>
                  <div><strong>Student:</strong> {payingRecord.students?.name}</div>
                  <div><strong>Period:</strong> Month {payingRecord.month}/{payingRecord.year}</div>
                  <div><strong>Amount:</strong> ${payingRecord.amount}</div>
                </div>
              ...
              <div className="modal-footer">
                <Button variant="outline" type="button" onClick={() => setPayingRecord(null)}>Cancel</Button>
